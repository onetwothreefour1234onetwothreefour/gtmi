...
**Visual language (Phase 4 Redesign).** The dashboard is editorial in tone — warm cream paper (`--paper`, `--paper-2`, `--paper-3`), Fraunces serif for display + body emphasis, Inter Tight for sans copy, JetBrains Mono for tabular data, oxblood (`#B8412A`) as the single attention accent, deep navy reserved for peer-review marginalia. Section dividers use full-bleed `<SpecimenPlate>` and chapter-style `<SectionPlate>` artefacts; provenance opens in a right-side drawer (Phase C) rather than a popover. The methodology page (Phase D) opens with a 64px Fraunces hero ending in oxblood-italic _"falsifiable"_, a live 5-cell stats strip (pillars, sub-factors, indicators, programmes scored, last updated — all DB-derived), `<SplitSpecimen>` and `<PillarsSpecimen>` reading live methodology weights, the new `<WeightTree>` server component (root → CME / PAQ → pillar → sub-factor → optional indicator hierarchy with `role="tree"` + per-node `aria-level`), per-pillar rationale blocks loading from `content/pillars/{A..E}.md`, and `<FalsifiabilityCommitments>` (six numbered editorial commitments). The country page (Phase D) presents `<CountryHeader>` (large serif country name, IMD rank/score, top-programme stat strip), `<CountryProgramsTable>` (editorial `table.gtmi` ranking the country's programmes), `<CountryRadar>` (multi-programme overlay — oxblood for the leader, navy for the rest, sr-only data table for screen readers), `<TaxTreatmentCard>` aggregating D.3.2 / D.3.3 across the country's programmes, and a country-stability section gated behind `NEXT_PUBLIC_STABILITY_ENABLED` until Phase 5 lights up live policy data. Pillar labels track the methodology vocabulary — Access / Process / Rights / Pathway / Stability — even where the original design copy used editorial alternates. Light-only — `next-themes` and dark-mode tokens were removed during Phase A.

**Internal tools (Phase E).** The editorial review queue at `/review` is auth-gated (Supabase magic-link) and rendered behind a black `<InternalBadge>` ribbon. Its header pairs a Fraunces "Pending review." with a live 4-cell stats strip — `in queue` / `SLA risk` (rows older than 3 days) / `avg age` / `auto-conf ≥ 0.9` — all computed in a single round trip against `field_values`. Filter chips (All / Pending / In review / Flagged / High confidence) persist in URL state. A `<BulkApproveDialog>` (Radix modal confirmation) approves every row that clears the gate — `extractionConfidence ≥ 0.85` AND `validationConfidence ≥ 0.85` AND validator did not flag the source sentence as a mismatch — in a single transaction that mirrors the change into `review_queue.status`. The queue table renders the design's 10 columns including a `<ProvenanceHighlight>`-rendered source sentence on every detail page. The public-facing changes audit at `/changes` mirrors the same editorial vocabulary: 5 filter tabs (All / Data / Methodology / Provenance / Countries) with severity-coloured diamond markers, Fraunces programme names, Δ PAQ deltas, and a graceful Phase 5 empty state for the current `policy_changes` reality (the table is empty under RLS until Phase 5 brings it online; the page activates with zero code change).
...
**Drawer presentation (Phase 4 Redesign / Phase C).** On the public programme detail page, the provenance affordance opens as a right-side drawer (Radix Dialog modal, 540px, focus-trapped, ESC-to-close, oxblood-ink left rule). The drawer renders the full ADR-007 schema in a single source card: character-offset-highlighted source sentence, char/page/sha/scrape grid, source URL, Tier 2 / country-substitute / derived-knowledge advisory notes when applicable, derived-inputs block when present, extraction + validation confidence bars, cross-check result, methodology version, reviewer + reviewer notes (when status === 'approved'), stability edge-case note. The Wayback "View archived version" link is rendered disabled with the ADR-008 Phase 5 tooltip. A single source card is rendered per the Q13 redesign decision; corroborating-source UI is reserved for when the data lights up.
...
### Phase 3 (coverage maximization) — ✅ COMPLETE (Phase 3.9 close, 2026-05-02)

Phases 3.1–3.8 closed Sessions 11–14. **Phase 3.9** (Sessions 15–17) closed the operational and coverage gaps surfaced by post-Phase 2 canaries before scaling extraction to the full cohort. 9 PRs (A–I) plus W22 follow-up shipped, covering: per-scrape GCS archive snapshots (W0), native-language scrape + translation (W2), cross-departmental authority registry (W3), per-program discovery hints (W5), weekly maintenance scrape (W6, ADR-023), discovery telemetry (W8), `extraction_attempts` history (W9), yield-ranked URL merge (W10), hash-based extraction short-circuit (W11), surgical re-run modes (W12), Stage-0 missing-fields brief (W14), country-agnostic anti-bot blocker detection + Wayback-first routing (W15+W16, ADR-024), in-memory PDF integration tests (W17), country-level D.2.4 / D.3.1 / D.3.3 + program-age E.1.3 + policy-history E.1.1 derives (W20+W21, ADR-025), URL-merge filter against `blocker_domains` (W22). **Total derive coverage: 12/48 fields.** Production-validated with NLD HSM (44/48) and JPN HSP (23/48 — bounded by ISA's all-paths anti-bot wall, captured in `blocker_domains`). Gate to Phase 5 is no longer "≥42/48 on canary programmes" — it is the readiness pass below.

### Phase 3.10 (pre-scale wiring + readiness pass) — IN PROGRESS

Goal: finalise the orchestration, surface every Phase 3.9 capability through admin / Trigger.dev / public surfaces, and verify operational readiness BEFORE scaling extraction to the full 85-programme cohort. No new methodology; no new mechanisms. Top-level deliverables (full sequencing in [IMPLEMENTATION_PLAN.md §Phase 3.10](IMPLEMENTATION_PLAN.md)): data hygiene (backfill `programs.launch_year` for cohort, audit `program_policy_history`), admin/review wiring (surface `blocker_domains` in /admin, validate derive provenance badges), Trigger.dev parity (the twin must exercise W15+W16+W22 + all 12 derives), cost + observability (Cloud Logging metric on blocker registrations, Trigger.dev cost projections), discovery hint cohort sweep, two cohort dry-runs (`--mode discover-only` then `--mode archive-first`), pre-scale gate.
