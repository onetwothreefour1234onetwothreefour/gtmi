# Global Talent Mobility Index (GTMI) — Methodology v5

> **Document status:** Canonical methodology specification v5. Supersedes v4 — plan restructured 27 Apr 2026; Phase 3.6 close-out 28 Apr 2026 added Stage 6.5 — Derive (ADR-016) and the self-improving sources table (ADR-015). Coverage maximization moved to Phase 3 (before the 5-country pilot). Calibration now deferred to Phase 5. Wayback archival deferred to Phase 6. Changes from v4 are marked `[NEW v5]`.

> **Last updated:** 28 Apr 2026 (Session 12 / Phase 3.6 close-out). **Phase 2 closed (tag `phase-2-complete`)**, **Phase 3.6 closed (branch `phase-3-recanary-prep`, 8 commits)**. AUS Skills in Demand 482 — Core (PAQ 13.72) and SGP S Pass (PAQ 18.11) scored deterministically and tagged `phase2Placeholder: true`; calibration deferred to Phase 5. Phase 3.6 added: ADR-015 self-improving sources table (Stage 0 write-back + URL merge); ADR-016 derived fields (Stage 6.5; A.1.2 and D.2.2 computed deterministically, route to /review); E.3.1 World Bank WGI Rule of Law direct-API; scrape thin-content threshold 300→1500 + Jina escalation; C.3.2 country-substitute column reconciliation; Tier 2 allowlist expanded to B.2.3 / B.2.4 / D.2.4 (ADR-013 amended). Field-aware content windowing replaces head-slice in extraction. Wave 2 enabled — full 48-field methodology coverage. Currency code preserved in `provenance.valueCurrency`. Batch extraction + extraction cache + scrape cache shipped. Wayback archival deferred to Phase 6 (ADR-008). 401 tests passing.
...
2. **Extract** — Claude extracts value, confidence, exact source sentence, character offsets. The extraction system prompt treats government-page list formats as explicit statements: bullet-point eligibility lists, condition blocks, numbered requirement lists, and table rows are all valid value sources. Strict per-field-type output format rules: numeric fields return the base value only (minimum of a range); categorical fields return a 1–5 word label; text fields return a 20-word max summary; count fields return an integer only. Field-aware content windowing (`utils/window.ts`) selects relevance-scored 2K chunks (200-char overlap) within a 30K budget keyed on `field_definitions.label`, with a 1500-char baseline prefix and 800-char baseline suffix; replaces the previous head-slice [NEW v4]. Batch extraction (`executeAllFields`) extracts all `ACTIVE_FIELD_CODES` from a single scrape in one LLM call (8K max-tokens, JSON array), merging across URLs by highest confidence per field, with a 30s inter-batch delay and an early-exit once every field reaches confidence ≥ 0.9 [NEW v4]. `extraction_cache` table memoizes results keyed on `sha256(contentHash + fieldKey + promptHash + WINDOW_VERSION)`; cache hits skip the LLM entirely [NEW v4]. Rate-limit retry: 3 attempts on HTTP 429 with exponential back-off (60s × attempt). Coverage-gap sentinels (`not_addressed` / `not_found`) are skipped at publish so absence is honest in coverage math [NEW v4].
   2.5. **Derive (ADR-016) [NEW v5]** — Between Stage 2 and Stage 3, a deterministic arithmetic step computes A.1.2 (salary as % of local median wage) and D.2.2 (total years to citizenship) from already-extracted input fields and static lookup tables (`scripts/country-median-wage.ts`, `scripts/fx-rates.ts`, `scripts/country-citizenship-residence.ts`). No LLM is invoked. Derived rows carry `extractionConfidence` 0.6 and route to human review before publication. Skip conditions (input field absent; FX or median-wage table miss; D.1.1 false / no PR pathway; citizenship-residence is null for the country) emit a one-line log and write no row — the missing-data penalty applies normally.
...
- **Derived fields (A.1.2 and D.2.2) [NEW v5 / ADR-016]**: A.1.2 and D.2.2 are computed after batch extraction. Both route to /review on confidence 0.6. If input fields are absent, the derived field stays empty and the missing-data penalty applies. The reviewer accepts/rejects the computation; auto-approval is impossible by construction (extractionConfidence is hard-coded below the 0.85 threshold).
...
- Derived-field computation (Stage 6.5). A.1.2 and D.2.2 are deterministic arithmetic over already-extracted inputs plus static lookup tables — no model is invoked, and the reviewer (not the model) accepts the computation [NEW v5 / ADR-016].
