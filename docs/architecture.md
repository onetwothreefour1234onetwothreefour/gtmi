...
| `scripts`             | `canary-run.ts` — full 7-stage pipeline runner; accepts `--country AUS\|SGP`; pre-fetches WGI score; bypasses LLM for E.3.2; cross-check hardcoded to `not_checked`. `wave-config.ts` — exports `WAVE_1_FIELD_CODES` (27), `WAVE_2_FIELD_CODES` (21), and `ACTIVE_FIELD_CODES` (consumers should import this; controlled by `WAVE_2_ENABLED` flag). `country-sources.ts` — `COUNTRY_LEVEL_SOURCES` registry (4 global sources), `getCountryLevelSources(fieldKey)`, `fetchWgiScore(iso3)`, `ISO3_TO_ISO2` mapping. `verify-provenance.ts` — read-only check that every field_values row has a complete ProvenanceRecord JSONB. `diag-empty-fields.ts` — root-cause classifier (TRUNCATION / LLM_MISS / ABSENT) for empty fields. |
...
**Wave configuration:** `scripts/wave-config.ts` exports `ACTIVE_FIELD_CODES = WAVE_1_FIELD_CODES ∪ (WAVE_2_ENABLED ? WAVE_2_FIELD_CODES : [])`. With `WAVE_2_ENABLED = true` (the current Phase 2 close-out default), the pipeline runs all 48 fields. Setting `WAVE_2_ENABLED = false` reverts to the original 27-field Wave 1 scope. All consumers — `canary-run.ts`, `extract-single-program.ts`, `run-paq-score.ts`, `diag-empty-fields.ts` — import `ACTIVE_FIELD_CODES` so a single flag flip changes scope everywhere.

**Field-aware content windowing:** `packages/extraction/src/utils/window.ts` exposes `selectContentWindow(content, fields, maxChars)`. The batch extraction path scores 2K-char chunks (200-char overlap) by per-field keyword match against `field_definitions.label`, then greedily fills a 30K budget while preserving a 1500-char baseline prefix and 800-char baseline suffix. Replaces the previous `content.slice(0, 30000)` truncation. Cache key in `extract.ts` includes a `WINDOW_VERSION` constant so changes to the windowing logic invalidate stale cache rows cleanly. When no field labels are passed (single-field legacy path), windowing falls back to head-slice — guaranteed never worse than pre-windowing behaviour.
...
- **Wave field config**: `scripts/wave-config.ts` exports `WAVE_1_FIELD_CODES` (27), `WAVE_2_FIELD_CODES` (21), and `ACTIVE_FIELD_CODES` (consumers import this). `WAVE_2_ENABLED = true` activates full 48-field coverage; `false` reverts to Wave 1 only. Canary, Trigger.dev job, scoring script, and diagnostic all consume `ACTIVE_FIELD_CODES`. Rollback = one flag change.
