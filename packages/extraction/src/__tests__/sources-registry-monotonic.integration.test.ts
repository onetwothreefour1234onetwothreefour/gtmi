...
describe('ADR-015 — sources registry grows monotonically across consecutive runs (live DB)', () => {
  afterAll(async () => {
  ...
  it('after two consecutive Stage 0 write-backs, run 2 sources are a superset of run 1', async () => {
    ...
    await db
      .delete(sources)
      .where(and(eq(sources.programId, testProgramId), like(sources.url, `%${TEST_URL_MARKER}%`)));
